...
text(Tree) ->
    {ok, Deep_atom@1} = case gleam@erlang@atom:from_string(<<"deep"/utf8>>) of
        {ok, Deep_atom} -> {ok, Deep_atom};
        _try ->
            erlang:error(#{gleam_error => assert,
                           message => <<"Assertion pattern match failed"/utf8>>,
                           value => _try,
                           module => <<"sources/master_of_malt"/utf8>>,
                           function => <<"text"/utf8>>,
                           line => 50})
    end,
    'Elixir.Floki':text(Tree, [{Deep_atom@1, gleam@dynamic:from(false)}]).
