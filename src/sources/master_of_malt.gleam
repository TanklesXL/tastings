...
pub fn endpoint() -> String {
  string.concat(["https://", master_of_malt_url])
}
...
type Keyword =
  List(tuple(Atom, Dynamic))

external fn text(tree: List(HTMLNode), Keyword) -> String =
  "Elixir.Floki" "text"

fn find_text(html: List(HTMLNode), query: String) -> Result(String, String) {
  assert Ok(deep_atom) = atom.from_string("deep")
  let no_deep_text = tuple(deep_atom, dynamic.from(False))
